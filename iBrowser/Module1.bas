...
Option Explicit
Public WinInfo(100) As WindowInfo
...
Public Function FreeID()
    Dim i As Integer
    For i = 1 To 100
        'MsgBox WinInfo(0).winID
        If WinInfo(i).tabID = 101 And WinInfo(i).winid = 101 Then
            FreeID = i
            Exit For
        End If
        If i = 100 Then MsgBox "You have opened the maximum number of windows. Please close one or more to open a new one.", vbOKOnly + vbCritical, "CyberSystem Error"
    Next
End Function
Public Function InitializeWinInfo()
    Dim i As Integer
    For i = 2 To 100
        WinInfo(i).tabID = 101
        WinInfo(i).winid = 101
